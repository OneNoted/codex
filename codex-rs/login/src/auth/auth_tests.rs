...
#[serial(codex_agent_identity)]
#[serial(codex_api_key)]
async fn enforce_login_restrictions_allows_matching_agent_identity_workspace() {
    let codex_home = tempdir().unwrap();
...
#[serial(codex_agent_identity)]
#[serial(codex_api_key)]
async fn enforce_login_restrictions_blocks_agent_identity_workspace_mismatch() {
    let codex_home = tempdir().unwrap();
    ...
    assert!(
        err.to_string().contains(
            "Login is restricted to workspace org_mine, but current agent identity belongs to org_another_org."
        )
    );
}
