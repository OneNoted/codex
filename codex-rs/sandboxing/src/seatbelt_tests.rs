...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    ...
    assert!(
        policy_text.contains("WRITABLE_ROOT_2_EXCLUDED_0")
            && !policy_text.contains("WRITABLE_ROOT_2_EXCLUDED_1"),
        "expected second explicit writable root .git carveout in policy:\n{policy_text}",
    );
...
(allow file-read*)
(allow file-write*
(require-all (subpath (param "WRITABLE_ROOT_0")) (require-not (literal (param "WRITABLE_ROOT_0_EXCLUDED_0"))) (require-not (subpath (param "WRITABLE_ROOT_0_EXCLUDED_0"))) (require-not (literal (param "WRITABLE_ROOT_0_EXCLUDED_1"))) (require-not (subpath (param "WRITABLE_ROOT_0_EXCLUDED_1"))) ) (require-all (subpath (param "WRITABLE_ROOT_1")) (require-not (literal (param "WRITABLE_ROOT_1_EXCLUDED_0"))) (require-not (subpath (param "WRITABLE_ROOT_1_EXCLUDED_0"))) ){tempdir_policy_entry}
)
...
"#,
    ...
    let mut expected_args = vec![
        ...
        format!("-DWRITABLE_ROOT_1={}", tmp_canonical.to_string_lossy()),
        format!(
            "-DWRITABLE_ROOT_1_EXCLUDED_0={}",
            tmp_canonical.join(".git").to_string_lossy()
        ),
    ];
    ...
    if let Some(p) = tmpdir_env_var {
        ...
        expected_args.push(format!(
            "-DWRITABLE_ROOT_2_EXCLUDED_0={}",
            PathBuf::from(&p).join(".git").to_string_lossy()
        ));
        expected_args.push(format!(
